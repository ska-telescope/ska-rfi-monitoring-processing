...
if  read_data_from_files ==1:
    time,time_data = read_phase0_data(directory,filenum='all')
    [freq,Ant1_pow] = fft_calc(time_data,1, 0)
    del time_data
...
#Power = np.abs(Ant1_fft[:,0:int(np.size(freq)/2)]*np.conjugate(Ant1_fft[:,0:int(np.size(freq)/2)]))
#del Ant1_fft
#freq = freq[0:int(np.size(freq)/2)]/1e6



#%% maximum values

#plt.plot(SKALA4_freq,10*np.log10(np.max(SKALA4_pow,0)))
plot_max_peak(SKALA4_freq,10*np.log10(np.max(SKALA4_pow,0)))
plt.grid(True,'both')
plt.title('Max values - Phase0 - SKALA4.0 - 20190331')

#%% percentiles
perc = 98
plot_max_peak(SKALA4_freq,10*np.log10(np.percentile(SKALA4_pow,perc,axis=0)))
plt.grid(True,'both')
plt.title(str(perc) + ' percentile - Phase0 - SKALA4.0 - 20190331')



#%% Waterfall plot

plot_waterfall(SKALA4_time/3600,SKALA4_freq,SKALA4_pow,'Spectrogram - Phase0 SKALA4.0 - 20190331')
...
time2,Data2 = total_power(SKALA4_time,SKALA4_freq,SKALA4_pow,fo,B,low_th,high_th,0)


plot_max_peak(time2,10*np.log10(Data2))
plt.grid(True,'both') 
plt.title('Phase0 - SKALA4.0 - 20190331 - Fo=' + str(fo) + ' MHz - B=' + str(B) + 'MHz')
